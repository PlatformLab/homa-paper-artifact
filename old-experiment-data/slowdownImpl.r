...
############### MedianSlowdown individual plots ####################
...
############### Tail slowdown, 3 workloads combined ####################
i <- 0
tailStretchPlot = list()
rhos <- c(76)
    ...
    for (rho in rhos) {
        ...
        if (workload == workloads[length(workloads)]) {
            theme_i <- theme_i +
                ...
                legend.text = element_text(size=1.2*textSize),
                legend.title = element_blank(),
                plot.margin=unit(c(0, 9, -1.6, -2.5),"cm"))
        } else if (workload == workloads[1]) {
            theme_i <- theme_i +
                theme(legend.position = "none",
                plot.margin=unit(c(0, 2.5, -1.6, 0),"cm"))
        } else {
            theme_i <- theme_i +
                theme(legend.position = "none",
                plot.margin=unit(c(0, 2.5, -1.6, -2.5),"cm"))
        }
...
############### Median slowdown, 3 workloads combined ####################
i <- 0
medianStretchPlot = list()
rhos <- c(76)
workloads <- c("w3", "w4", "w5")
for (workload in workloads) {
    for (rho in rhos) {
        ...
        yLab = ''
        if (workload == workloads[1]) {
            yLab = 'Median Slowdown (Log Scale)'
        }

        yLimit <- 150
        yLimit <- as.integer(ceiling(min(max(tmp$MedianStretch), yLimit)))
        ...
        medianStretchPlot[[i]] <- medianStretchPlot[[i]] +
            ...
            scale_x_continuous(limits = c(0,101), breaks = brks,
                labels=brkLabels, expand = c(0, 0)) +
            theme_i +
            scale_y_log10(breaks=c(1,2,3,5,10,20,40,100,1000,yLimit)) +
            coord_cartesian(ylim=c(1, yLimit)) +
            labs(title = plotTitle, y = yLab, x = xLab) +
            scale_color_manual(values = tTypesColors)
...
pdf(sprintf("impl/slowdownImpl_medianCombined.pdf", workload, rho),
    width=54, height=16)
args.list <- c(medianStretchPlot, list(ncol=i), list(widths=c(3.15,3,3.5)))
do.call(grid.arrange, args.list)
dev.off()
